...
export class BilibiliScraper {
    ...
    static async fetchVideoData(url: string): Promise<VideoData> {
        ...
        if (subtitleList.length === 0) {
            ...
            subtitleList = await this.fetchPlayerSubtitleList(info.bvid, info.cid, headers);
        }
        ...
        if (subtitleList.length === 0) {
            ...
            if (primarySub) {
                ...
                transcript = await this.downloadSubtitle(primarySub.subtitle_url, primarySub.lan, headers);
            }
            ...
            if (primarySub === enSub && zhSub) {
                ...
                translatedTranscript = await this.downloadSubtitle(zhSub.subtitle_url, zhSub.lan, headers);
            } else if (primarySub === zhSub && enSub) {
                console.log(`[LinguaSync] Downloading secondary (translated) subtitle: ${enSub.lan_doc}...`);
                translatedTranscript = await this.downloadSubtitle(enSub.subtitle_url, enSub.lan, headers);
            }
        }
     ...
     */
    private static async fetchPlayerSubtitleList(bvid: string, cid: number, headers: any): Promise<any[]> {
        try {
            const playerUrl = `https://api.bilibili.com/x/player/v2?bvid=${bvid}&cid=${cid}`;
            const res = await requestUrl({ url: playerUrl, headers });
            const data = res.json;
    ...
    private static async downloadSubtitle(url: string, langCode: string, headers?: any): Promise<TranscriptLine[]> {
        ...
        try {
            ...
            console.error('[LinguaSync] Failed to download subtitle:', e);
            return [];
        }
    }
}
